...
* UHCL ID: 1630300
* 
* Filename: ShellvacumarP1GoldServer.java
*
* Purpose: 
...
* Arguments: (1) Port
* 
* How to Run: ShellvacumarP1GoldServer 10300
* 
***********************************************************************/
import java.io.*;
import java.net.*;
...
class ShellvacumarP1GoldServer implements Runnable {
	...
	public ShellvacumarP1GoldServer(int port, String itemf) throws SocketException {
	...
	@Override
	public void run() {
		System.out.println("Threaded Gold Group Server is Running  ");
		try {
	...
	public static void main(String[] args) throws Exception {
		...
		String itemListFile = Constants.GOLD_ITEM_LIST_FILE;
		
		try {
			ShellvacumarP1GoldServer server = new ShellvacumarP1GoldServer(port, itemListFile);
		...
		if (purchaseItem.equals(Constants.ALLITEMS)) {
			...
			if(points >= (purchaseQty * Integer.parseInt(itemPoints[userIndex]))) {
				...
				if(purchaseQty <= Integer.parseInt(itemQtys[userIndex])) {
					...
					System.out.println("Gold Group Product List File Updated...");
