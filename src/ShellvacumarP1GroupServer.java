...
* UHCL ID: 1630300
* 
* Filename: ShellvacumarP1GroupServer.java
*
* Purpose: 
...
* Arguments: (1) Port
* 
* How to Run: ShellvacumarP1GroupServer 10300
* 
***********************************************************************/
import java.io.*;
import java.net.*;
...
class ShellvacumarP1GroupServer implements Runnable {
	...
	public ShellvacumarP1GroupServer(int port) throws SocketException {
		socket = new DatagramSocket(port);
	}

	@Override
	public void run() {
		System.out.println("Threaded Group Server is Running  ");
		try {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			ShellvacumarP1GroupServer server = new ShellvacumarP1GroupServer(port);
	...
	private void dataGramService() throws NumberFormatException, Exception {
		while (true) {
			...
			if (receivedStringSplitArray[0].equals(Constants.NOTHING)) {
				System.out.println("Processing.....  Item List Request");
				if(receivedStringSplitArray[1].equals(Constants.SILVER)) {
					itemFile = Constants.SILVER_ITEM_LIST_FILE;
				} else if(receivedStringSplitArray[1].equals(Constants.GOLD)) {
					itemFile = Constants.GOLD_ITEM_LIST_FILE;
				} else {
					itemFile = Constants.PLATINUM_ITEM_LIST_FILE;
				}
				...
				if(receivedStringSplitArray[3].equals(Constants.SILVER)) {
					itemFile = Constants.SILVER_ITEM_LIST_FILE;
				} else if(receivedStringSplitArray[3].equals(Constants.GOLD)) {
					itemFile = Constants.GOLD_ITEM_LIST_FILE;
				} else {
					itemFile = Constants.PLATINUM_ITEM_LIST_FILE;
				}
		...
		if (purchaseItem.equals(Constants.ALLITEMS)) {
			...
			if(points >= (purchaseQty * Integer.parseInt(itemPoints[userIndex]))) {
				...
				if(purchaseQty <= Integer.parseInt(itemQtys[userIndex])) {
					...
					System.out.println(itemFile + " File Updated...");
