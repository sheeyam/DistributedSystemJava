...
* UHCL ID: 1630300
* 
* Filename: ShellvacumarP1SilverServer.java
*
* Purpose: 
...
* Arguments: (1) Port
* 
* How to Run: ShellvacumarP1SilverServer 10300
* 
***********************************************************************/
import java.io.*;
import java.net.*;
...
class ShellvacumarP1SilverServer implements Runnable {
	...
	public ShellvacumarP1SilverServer(int port, String itemf) throws SocketException {
	...
	@Override
	public void run() {
		System.out.println("Threaded Silver Group Server is Running  ");
		try {
	...
	public static void main(String[] args) throws Exception {
		...
		String itemListFile = Constants.SILVER_ITEM_LIST_FILE;
		
		try {
			ShellvacumarP1SilverServer server = new ShellvacumarP1SilverServer(port, itemListFile);
		...
		if (purchaseItem.equals(Constants.ALLITEMS)) {
			...
			if(points >= (purchaseQty * Integer.parseInt(itemPoints[userIndex]))) {
				...
				if(purchaseQty <= Integer.parseInt(itemQtys[userIndex])) {
					...
					System.out.println("Silver Group Product List File Updated...");
